...
void __declspec(naked) x86_instruction_length_end(void){
	__asm {
		nop
		retn
	}
...
bool run_test (const char* testName, void* codePtr, int desiredResult) {
...
int main (int argc, char* argv[]) {
	...
	int j,k=0;
	unsigned char* codePtr = (unsigned char*)x86_instruction_length;

	printf("Doing built-in test...\n");

	// run opcode tests
	for (j = 0; j < sizeof(testData)/sizeof(testData[0]); j++) {
		if (run_test(testData[j].testName, testData[j].codePtr,
	...
	printf("Iterating instruction lengths of self...\n");
	
	k=0;
	while ((unsigned long)codePtr < (unsigned long)x86_instruction_length_end) {
